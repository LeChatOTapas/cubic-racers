...
public class FakeBoxUseMessage {
    ...
    public FakeBoxUseMessage(double x, double y, double z, boolean isLobShot, float direction, float kartSpeed) {
        this.x = x;
        this.y = y;
        this.z = z;
    ...
    public static FakeBoxUseMessage decode(FriendlyByteBuf buffer) {
        return new FakeBoxUseMessage(buffer.readDouble(), buffer.readDouble(), buffer.readDouble(), buffer.readBoolean(), buffer.readFloat(), buffer.readFloat());
    }
    ...
    public static void handle(FakeBoxUseMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        context.enqueueWork(() -> {
            ...
            FakeBox entity = new FakeBox(KartItemsInit.FAKE_BOX.get(), level);
